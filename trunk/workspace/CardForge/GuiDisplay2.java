...
public class GuiDisplay2 extends javax.swing.JFrame implements Display
{
    ...
    {
      new Gui_MultipleBlockers(blockers, damage, this);
    }
	...
	int check = JOptionPane.showConfirmDialog(null, new JScrollPane(list), message, JOptionPane.OK_CANCEL_OPTION, JOptionPane.INFORMATION_MESSAGE);
	...
	if(c.isLand())
	    cdLabel1.setText(c.getName());
	else
	    cdLabel1.setText(c.getName() +"  - " +c.getManaCost());
	...
	if(c.isCreature())
        ...
        if(c.isPlaneswalker())
          cdLabel4.setText("Assigned Damage: " +c.getAssignedDamage());
