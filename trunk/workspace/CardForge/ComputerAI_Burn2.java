    ...
    Runnable run = new Runnable()
    {
      public void run()
      {
        synchronized(ComputerAI_Burn2.this){
          if(AllZone.Stack.size() == 0 && AllZone.Phase.is(Constant.Phase.Main1, Constant.Player.Computer))
          {
...
//              System.out.println("Spell List - " +list);
            ...
            if(list.size() != 0)
            {
              ...
              if(ComputerUtil.canPlay(sa) && sa.canPlay())
              {
                ...
                throw new RuntimeException("ComputerAI_Burn2 error, cannot pay for spell " +sa.getSourceCard() +" is spell? " +sa.isSpell() +"  sa.canPlayAI()-" +sa.canPlayAI()  +" sa.canPlay()-" +sa.canPlay()  +" CardUtil.getConvertedManaCost(sa)-" +CardUtil.getConvertedManaCost(sa) +" ComputerUtil.getAvailableMana().size()-" +ComputerUtil.getAvailableMana().size());
            }//list.size != 0
...
//      thread.start();
      }catch(Exception ex) {ex.printStackTrace(); throw new RuntimeException("ComputerAI_Burn : main1() error, " +ex);}
