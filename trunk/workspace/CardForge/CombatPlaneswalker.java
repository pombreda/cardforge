...
public class CombatPlaneswalker
{
  ...
  public void setAssignedDamage()
  {
    ...
    for(int i = 0; i < attacking.size(); i++)
    {
      ...
      {
        block.get(0).setAssignedDamage(attacking.get(i).getAttack());

        //trample
        int trample = attacking.get(i).getAttack() - block.get(0).getDefense();
        if(attacking.get(i).getKeyword().contains("Trample") && 0 < trample)
          this.addDefendingDamage(trample);

        trample = block.get(0).getAttack() - attacking.get(i).getDefense();
      ...
      {
        setAssignedDamage(block, attacking.get(i).getAttack());
      }
      else//human
      {
        GuiDisplay2 gui = (GuiDisplay2) AllZone.Display;
        gui.assignDamage(block, attacking.get(i).getAttack());
      }
    }//for
